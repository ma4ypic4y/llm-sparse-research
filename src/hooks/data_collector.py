...
def _lerp_between(min: float, max: float, between_count: int) -> list[float]:
    """
...
def _get_weights_distribution_statistics(param: torch.Tensor, try_unique_count: bool = False, force_exact_quantiles: bool = False) -> dict:
    """
    Computes distribution statistics for a single parameter tensor.

    Args:
        param: A torch tensor representing the parameter.
        try_unique_count: Whether to try making weights distribution through unique counts.
    ...
    between_dots, between_dots_cnt = None, None

    if try_unique_count:
        between_dots_, between_dots_cnt_ = torch.unique(weights_magnitudes, return_counts=True)
        if len(between_dots_) <= 13:
            between_dots, between_dots_cnt = between_dots_.tolist(), between_dots_cnt_.tolist()

    if between_dots is None or between_dots_cnt is None:
    ...
    if weights_magnitudes.numel() < 100_000 or force_exact_quantiles:
        ...
        total_dots_cnt = weights_magnitudes.numel()
        accum_dots_cnt = 0
        for dot, dot_cnt in zip(between_dots, between_dots_cnt):
            accum_dots_cnt += dot_cnt
            if accum_dots_cnt / total_dots_cnt > 0.25 and quantile_25 > dot:
                quantile_25 = dot
            if accum_dots_cnt / total_dots_cnt > 0.5 and quantile_50 > dot:
                quantile_50 = dot
            if accum_dots_cnt / total_dots_cnt > 0.75 and quantile_75 > dot:
                quantile_75 = dot
...
class BaseCollector(TrainerCallback):
    """
    ...
    def __init__(
            ...
            output_dir: str | None = "./collector_output",
            warmup_steps: int = 0,
    ):
    ...
    def _dump_data(self, state: TrainerState):
        """
    ...
    def on_optimizer_step(self, args, state, control, **kwargs):
        ...
        if not self.is_initialized and self.model is not None:
...
class MasksStatisticsCollector(BaseCollector):
    ...
    def __init__(
            ...
            collect_frequency: int = 1,
            dump_frequency: int = -1,
            output_dir: str | None = "./masks_collector_output",
            warmup_steps: int = 0,
    ):
        """
...
class WeightsStatisticsCollector(BaseCollector):
    ...
    def __init__(
            ...
            force_exact_quantiles: bool = False,
            trackable_layers: list[str] | None = None,
            collect_frequency: int = 1,
            dump_frequency: int = -1,
            output_dir: str | None = "./weights_statistics_collector_output",
            warmup_steps: int = 0,
    ):
        """
        ...
        Args:
            ...
            force_exact_quantiles: Whether to force exact quantiles computation. (default: False)
        ...
        self.force_exact_quantiles = force_exact_quantiles
    ...
    def _collect_data(self):
        ...
        for name in self.trackable_layers_names:
            ...
            weights_statistics[name].update({
                'weights_distribution': _get_weights_distribution_statistics(param, force_exact_quantiles=self.force_exact_quantiles),
                'gradients_distribution': _get_weights_distribution_statistics(grad, force_exact_quantiles=self.force_exact_quantiles) if grad is not None else None
            })
